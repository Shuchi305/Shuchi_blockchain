...
class Blockchain:
    ...
    def add_transaction(self,sender,receiver,cloth,quantity,quality):    #multiple job:-
        self.transactions.append({'sender':sender,
                                  'receiver':receiver,
                                  'cloth':cloth,
                                  'quantity':quantity,
                                  'quality':quality})         #adding trasaction into pool
...
def add_transaction():
    ...
    transaction_keys =['sender','receiver','cloth','quantity','quality']    #mandatory keys for transactions
    
    if not all(key in json for key in transaction_keys):   #validating the presence of required keys
        return 'Some Elements are missing',400
    
    index = blk.add_transaction(json['sender'],json['receiver'],json['cloth'],json['quantity'],json['quality'])  #adding transaction and accessing index of next block
